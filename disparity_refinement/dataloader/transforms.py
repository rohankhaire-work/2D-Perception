...
class ToTensor(object):
    ...
    def __call__(self, input):
        ...
        if len(input.shape) == 3:
            return torch.from_numpy(input.copy()).permute(2, 0, 1).type(self.dtype) / self.divider
...
class Resize(object):
    ...
    def __call__(self, inputs):
        ...
        inputs_ = resize(inputs_, output_shape=self.imsize,
    ...
    def __call__(self, inputs):
        if not (len(inputs.shape) >= 3 and inputs.shape[2] != 1):
            inputs -= inputs.reshape(-1,
                                     inputs.shape[2]).mean(axis=0)[None, None, :]
            inputs /= inputs.reshape(-1,
                                     inputs.shape[2]).std(axis=0)[None, None, :]
