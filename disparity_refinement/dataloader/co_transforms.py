    ...
    def __call__(self, inputs, targets):
        ...
        if isinstance(targets, list):
            for target_id, target_ in enumerate(targets):
                targets[target_id] = self.__randomCrop__(
                    target_, x1, y1, tw, th)
        else:
...
class Scale(object):
    ...
    def __ScalingInputs__(self, input_):
        ...
        if input_.shape[2] == 3:
            input_ = cv2.resize(input_, None, fx=self.scale,
                                fy=self.scale, interpolation=self.interp)
        else:
            input_ = self.scale * \
                cv2.resize(input_, None, fx=self.scale,
                           fy=self.scale, interpolation=self.interp)
    ...
    def __call__(self, inputs, targets):
        ...
        self.scale = self.scale_min + \
            (self.scale_max - self.scale_min) * random.random()
